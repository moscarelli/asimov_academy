...
# --- Copilot Semantic Evaluation for Business Rule Extraction ---
...
for model in passing_health:
    ...
    for lang, code in CODE_EXAMPLES.items():
        ...
        is_accurate = copilot_semantic_eval(answer, BUSINESS_RULE)
...
# Load business rule from markdown
with open(os.path.join(CODE_DIR, 'business_rule.md'), encoding='utf-8') as f:
    business_rule_text = f.read().strip().split('\n', 1)[-1].strip()

# Write results to markdown with Copilot semantic accuracy
md_lines = ["# Ollama Model Evaluation Results", ""]
md_lines.append("## Health Check Results\n| Model | Step | Time (s) | Error | Response |\n|-------|------|----------|-------|----------|")
for r in health_results:
